...
class SimpleAgent(BaseAgent):
    ...
    def on_quote(self, market_id: str, book_state: pd.Series):
        ...
        if total_trades_num > self.total_trades[self.market_id_index[market_id]]:
            for order in self.market_interface.get_filtered_orders(market_id, status="ACTIVE", side="buy"):
            ...
            for order in self.market_interface.get_filtered_orders(market_id, status="ACTIVE", side="sell"):
                self.market_interface.cancel_order(order)
            # [self.market_interface.cancel_order(order) for order in self.market_interface.get_filtered_orders(market_id, status="ACTIVE", side="buy")]
            # if market_id == "BASF":
            # print("########################")
                ...
                if self.trading_phase:
                    if not self.market_interface.get_filtered_orders(market_id, status="ACTIVE"):
                        if self.assigned_guess[market_id] == "UP":
                            self.market_interface.submit_order(market_id,"buy",math.floor(self.min_quant_bid[market_id] * 1.5),book_state["L1-BidPrice"])
                            self.market_interface.submit_order(market_id,"sell",self.min_quant_bid[market_id],book_state["L2-AskPrice"])
                        elif self.assigned_guess[market_id] == "DOWN":
                            self.market_interface.submit_order(market_id, "sell", math.floor(self.min_quant_bid[market_id] * 1.5), book_state["L1-AskPrice"])
                            self.market_interface.submit_order(market_id, "buy", self.min_quant_bid[market_id], book_state["L2-BidPrice"])
                        else:
                            pass
        ...
        # less exposure than 95% left:
            ...
            if not self.cancel_all:
                ...
                for order in self.market_interface.get_filtered_orders(status="ACTIVE"):
                    order.cancel()
                self.cancel_all = True
                #print("all orders cancelt!")
            if not self.market_interface.get_filtered_orders(status="ACTIVE"):     # maybe (market_id, status="ACTIVE")
...
result_per_Share.to_csv("./random_guess_with_correction/results_per_share_session123456.csv")
result_matrix.to_csv("./random_guess_with_correction/result_matrix_session123456.csv")
result_exposure.to_csv("./random_guess_with_correction/result_exposure_session123456.csv")
random_guesses.to_csv("./random_guess_with_correction/random_guesses_random_correction_session123456.csv")

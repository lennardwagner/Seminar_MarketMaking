...
class SimpleAgent(BaseAgent):
    ...
    def on_quote(self, market_id: str, book_state: pd.Series):
        ...
        if len(self.last_60_ticks[self.market_id_index[market_id]]) == 60:
            ...
            self.last_60_ticks[self.market_id_index[market_id]].append(
        ...
        if (len(self.last_60_ticks[self.market_id_index[market_id]]) == 60 and \
            ...
            if market_id == "Adidas":
                ...
                self.Allianz_ticks = pd.DataFrame(
                    columns=[str(i) for i in range(0, 60)])
                ...
                self.Continental_ticks = pd.DataFrame(
                    columns=[str(i) for i in range(0, 60)])
                ...
                self.Covestro_ticks = pd.DataFrame(
                    columns=[str(i) for i in range(0, 60)])
                ...
                self.Daimler_ticks = pd.DataFrame(
                    columns=[str(i) for i in range(0, 60)])
                ...
                self.DeutscheBank_ticks = pd.DataFrame(
                    columns=[str(i) for i in range(0, 60)])
                ...
                self.DeutscheBoerse_ticks = pd.DataFrame(
                    columns=[str(i) for i in range(0, 60)])
    ...
    def on_time(self, timestamp: pd.Timestamp, timestamp_next: pd.Timestamp):
        ...
        if not self.assigned_guess:
            self.assigned_guess = {key: "" for key in
...
if __name__ == "__main__":
    ...
    backtest.run_episode_list(identifier_list=identifier_list,
                              episode_list=[
                                  ...
                                  (
                                  "2021-02-05T08:00:00", "2021-02-05T08:15:00",
                                  "2021-02-05T09:15:00"),
                                  ...
                                  # ("2021-02-26T14:00:00", "2021-02-26T14:15:00", "2021-02-26T15:15:00"),
